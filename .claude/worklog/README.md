...
- [2026-09-07 the quacc[defects] cluster](2026-09-07-quacc-defects-cluster.md) — ten packages ending the materials chain; three bugs eval cannot see — vise's `boltztrap2` console script, doped's xdist-hostile fixtures, and a shipped `#!/bin/bash` in shakenbreak that fails *silently*; `stdenv.shell` for build-time scripts and `runtimeShell` for shipped ones; `disabledTests` is a substring match; an unfree top-level attribute is an eval throw, not a skip.
