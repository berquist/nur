...
| Which six of quacc's thirteen extras are check inputs, and why not the other seven? | `pkgs/quacc/default.nix` (`nativeCheckInputs`) |
| Why does quacc take `fairchem` — torch and all — for three tests, when `mlip` stays out? | `pkgs/quacc/default.nix` (the `fairchem` paragraph above `nativeCheckInputs`) |
...
| `quacc` | atomate2 follow-on | **done**; `pkgs/quacc`. Core `dependencies` all satisfied. Extras done: `dask`, `defects`, `fairchem`, `jobflow`, `mp`, `mlip`, `parsl`, `phonons`, `prefect`, `ray`, `redun`, `sella`, `tblite`. Missing: `torchsim` alone, and that is NVIDIA-blocked rather than unpackaged. Tests: the whole of upstream's `tests/core`, less `dftb_recipes`, `psi4_recipes`, `torchsim_recipes`, the two `jenkins` directories and three phonon tests that hit a QE bug — `defects`, `fairchem`, `mp`, `phonons`, `sella` and `tblite` are check inputs, and so are `openbabel-bindings` and a real `quantum-espresso`. Two modules run in a second pytest process; see `postCheck` |
