...
The role must be one of the seven QCFractal defines in
`qcfractal/components/auth/role_permissions.py`. There is no `user`:

| Role | For |
|---|---|
| `admin` | everything, including user management |
| `maintain` | as `admin` minus role/permission changes |
| `monitor` | read-only, plus the access log and server errors |
| `submit` | read, plus adding and modifying records, datasets and projects |
| `read` | read-only |
| `compute` | claiming and returning tasks, and registering a manager — nothing else |
| `anonymous` | unauthenticated access, when the server allows it |
...
same time, so ordinary use never involves the worker's credentials. That one wants `submit`,
which is the role that can add records:

```sh
sudo qcfractal-manage user add me --role submit
```

Unlike the worker's, this password is one you have to keep, so let it print rather than piping
it into a file. An invalid role fails *after* the CLI has otherwise validated everything, with
`InvalidRolenameError` and a message that does not list the alternatives — check the table
